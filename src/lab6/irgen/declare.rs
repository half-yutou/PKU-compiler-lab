...
use crate::lab6::irgen::{calc, vars, IRGen};
            ...
            Decl::Var(var_decl) => {
                for def in &var_decl.var_def_list {
                    ...
                    if let Some(init_val) = &def.init_val {
                        let init_value = generate_var_exp(&init_val.exp, func_data, &mut self.scope_stack, current_bb);
...
pub fn generate_var_exp(var_exp: &Exp, func_data: &mut FunctionData, scope_stack: &mut ScopeStack, current_bb: BasicBlock) -> Value {
    vars::generate_exp(var_exp, func_data, scope_stack, current_bb)
}
